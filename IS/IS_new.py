...
for dataset_path in paths:
    ...
    print(f"Inception Score for {dataset_path}: {is_value}")

    # 生成基于路径的输出文件名
    sanitized_path = dataset_path.replace('/', '_').replace(':', '')
    output_file = f'/share/DavidHong/code/quantitative/IS/IS_result_{sanitized_path}.json'

    # 将结果保存到JSON文件
    result = {
        'path': dataset_path,
        'inception_score': float(is_value)
    }
